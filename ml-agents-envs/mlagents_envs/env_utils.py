...
def validate_environment_path(env_path: str) -> Optional[str]:
    ...
    if not (glob.glob(env_path) or glob.glob(f"{env_path}.*")):
        return None

    cwd = os.getcwd()
    launch_string = None
    true_filename = os.path.basename(os.path.normpath(env_path))
    if get_platform() in ["linux", "linux2"]:
        candidates = glob.glob(f"{os.path.join(cwd, env_path)}.x86_64")
        if not candidates:
            candidates = glob.glob(f"{os.path.join(cwd, env_path)}.x86")
        if not candidates:
            candidates = glob.glob(f"{env_path}.x86_64")
        if not candidates:
            candidates = glob.glob(f"{env_path}.x86")
        if not candidates and os.path.isfile(env_path):
            candidates = [env_path]
        if candidates:
        ...
        candidates = glob.glob(
            os.path.join(
                cwd, f"{env_path}.app", "Contents", "MacOS", true_filename
            )
        )
        if not candidates:
            candidates = glob.glob(
                os.path.join(
                    f"{env_path}.app", "Contents", "MacOS", true_filename
                )
            )
        if not candidates:
            candidates = glob.glob(
                os.path.join(cwd, f"{env_path}.app", "Contents", "MacOS", "*")
            )
        if not candidates:
            candidates = glob.glob(
                os.path.join(f"{env_path}.app", "Contents", "MacOS", "*")
            )
        if candidates:
            launch_string = candidates[0]
    elif get_platform() == "win32":
        candidates = glob.glob(os.path.join(cwd, f"{env_path}.exe"))
        if not candidates:
            candidates = glob.glob(f"{env_path}.exe")
        if not candidates:
