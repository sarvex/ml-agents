...
def _unwrap_batch_steps(batch_steps, behavior_name):
    ...
    if decision_batch.action_mask is not None:
        obs |= {
            agent_id: {
                ...
                "action_mask": [
                    mask[i] for mask in decision_batch.action_mask
                ],
            }
    ...
    dones = {agent_id: True for agent_id in termination_id} | {
        agent_id: False for agent_id in decision_id
    }
    rewards = {
        agent_id: termination_batch.reward[i]
        for i, agent_id in enumerate(termination_id)
    } | {
        agent_id: decision_batch.reward[i]
        for i, agent_id in enumerate(decision_id)
    }
    cumulative_rewards = dict(rewards)
    infos = {
        agent_id: {
            "behavior_name": behavior_name,
            "group_id": decision_batch.group_id[i],
            "group_reward": decision_batch.group_reward[i],
        }
        for i, agent_id in enumerate(decision_id)
    }
    for i, agent_id in enumerate(termination_id):
